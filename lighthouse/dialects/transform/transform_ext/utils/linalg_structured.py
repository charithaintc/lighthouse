...
The C++ ``linalg::LinalgOp`` interface exposes indexing maps, iterator types,
loop ranges and body block arguments per *operand*. The Python bindings expose
only the raw ``indexing_maps`` / ``iterator_types`` attributes, so the
per-operand queries the reduction-fusion analysis relies on are rebuilt here.
...
The bindings' ``ir.OpOperand`` carries only ``owner``/``operand_number`` -- it
cannot read or write the operand's value -- so `Operand` below stands in for
C++'s ``OpOperand *``, which the analysis passes around to identify *which*
operand a map or block argument belongs to.
"""
...
class Operand:
    """A reference to one operand of an op: the Python stand-in for ``OpOperand *``.

    Holds the owning op and the operand index, and reads/writes the operand
    value through them. Two references are equal when they name the same operand
    of the same op, so they can be used as dict keys and compared like the C++
...
def is_dps_init(op: ir.Operation | ir.OpView, operand: Operand) -> bool:
    """Whether `operand` is a DPS init (``outs``) operand of `op`."""
    ov = opview(op)
    return operand.index >= len(ov.operands) - num_dps_inits(ov)
