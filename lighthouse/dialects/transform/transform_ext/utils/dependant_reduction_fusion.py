...
original body is cloned into it, and the fused ops are appended. The C++ original
instead calls ``scf::tileAndFuseConsumer`` twice and repairs its output (the
utility tiles only along dims the loop already yields, leaving the fused ops at
full reduction extent). That utility is not reachable from Python, and building
the target shape directly is both shorter and more predictable.
...
accumulator), so nothing may read it off the loop and
``--remove-dead-values`` unwinds it. See ``needs_elementwise_clone``.
"""
