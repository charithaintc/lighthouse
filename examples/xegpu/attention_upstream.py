...
"""
XeGPU attention benchmark using only upstream tiling and fusion transforms.

Same payload as `fused_attention.py`, but the schedule is restricted to
transforms available in upstream MLIR (no `replace_with_fused_attention`
extension). See `attention_upstream_schedule` for what that costs.
"""
...
from lighthouse.schedule.xegpu import attention_upstream_schedule, xegpu_to_binary


def attention_complexity(Z: int, H: int, n_ctx: int, n_head: int, nbytes: int):
    """
...
class XeGPUAttentionUpstream:
    """
    Attention workload on XeGPU, scheduled with upstream transforms only. This workload starts with standard attention
    ...
    def get_complexity(self) -> tuple[int, int, int]:
        ...
        return attention_complexity(self.Z, self.H, self.n_ctx, self.n_head, nbytes)
...
def parse_cli():
    parser = argparse.ArgumentParser(
        description="Attention using MLIR XeGPU with upstream-only transforms",
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Tile size for the kv reduction dimension (K/V sequence length)",
    )
    parser.add_argument(
        "--softmax-kv-tile-size",
        type=int,
        default=0,
        help="Tile size for the kv dimension of the max/sum passes "
        "(0 uses --kv-tile-size).",
    )
    parser.add_argument(
        "--unroll-factor",
        type=int,
        default=1,
        help="Unroll factor for the kv reduction loops (1 disables).",
    )
    parser.add_argument(
        "--nb-prefetch",
        type=int,
...
if __name__ == "__main__":
    ...
    params = {
        ...
        "softmax_kv_tile_size": args.softmax_kv_tile_size,
    ...
    with ir.Context(), ir.Location.unknown():
        ...
        wload = XeGPUAttentionUpstream(
            Z=Z, H=H, n_ctx=n_ctx, n_head=n_head, dtype=dtype
        )
